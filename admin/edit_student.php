...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $pdo->prepare("UPDATE students SET last_name=?, first_name=?, middle_name=?, 
    matric_number=?, confirm_password=?, level=?, faculty=?, department=?, gender=?, phone=? 
    WHERE id=?");
    $stmt->execute([$last_name, $first_name, $middle_name, $matric_number, $confirm_password,
     $level, $faculty, $department, $gender, $phone, $id]);
...
<div class="container mt-5">
    <h2>Edit Student: <?= htmlspecialchars($student['last_name']) ?>  <?= htmlspecialchars($student['first_name']) ?></h2>
    <form method="POST">
        <div class="mb-3">
        ...
        <div class="mb-3">
            ...
            <input type="text" name="middle_name" class="form-control" value="<?= htmlspecialchars($student['middle_name']) ?>" required>
        </div>
        <div class="mb-3">
        ...
        <div class="mb-3">
            ...
            <input type="text" name="confirm_password" class="form-control" value="<?= htmlspecialchars($student['confirm_password']) ?>" required>
        </div>
        ...
        <div class="mb-3">
            ...
            <select name="faculty" class="form-select" required>
                <?php foreach ($faculties as $fac): ?>
                    <option value="<?= $fac['id'] ?>" <?= $student['faculty'] == $fac['id'] ? 'selected' : '' ?>><?= $fac['name'] ?></option>
                <?php endforeach; ?>
            </select>
        </div>
        <div class="mb-3">
